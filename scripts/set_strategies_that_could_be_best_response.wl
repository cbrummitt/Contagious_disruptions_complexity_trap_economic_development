...
(* ::Text:: *)
(*This function strategiesThatCouldBeBestResponsesFaster computes the set of strategies {(Subscript[m, 1],Subscript[\[Tau], 1]), (Subscript[m, 2],Subscript[\[Tau], 2]), ...} that could potentially be a best response:*)


(* ::Input::Initialization:: *)
strategiesThatCouldBeBestResponses[{\[Alpha]_,\[Beta]_},F_]/;0<=F<=1&&\[Alpha]>0&&0<=\[Beta]<=1:=
Which[
(* first line of Equation SI-12 in the Supplement *)
F==0,
{{0,0}}, 

(* second line of Equation SI-12 in the Supplement *)
F==1,
{{Ceiling[(\[Alpha]/\[Beta])^(1/(\[Beta]-1))],Ceiling[(\[Alpha]/\[Beta])^(1/(\[Beta]-1))]},{Floor[(\[Alpha]/\[Beta])^(1/(\[Beta]-1))],Floor[(\[Alpha]/\[Beta])^(1/(\[Beta]-1))]}}, 

(* third line of Equation SI-12 in the Supplement *)
True,
Module[{mMax},
